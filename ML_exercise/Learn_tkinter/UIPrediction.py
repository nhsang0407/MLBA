...
class UIPrediction:
    ...
    def create_ui(self):
        ...
        self.choose_dataset_button = Button(
            choose_dataset_panel, text="1. Pick Dataset", width=10, command=self.do_pick_data
        )
        self.view_dataset_button = Button(
            choose_dataset_panel, text="2. View Dataset", width=20, command=self.do_view_dataset
        )
        ...
        training_rate_entry = Entry(training_rate_panel, textvariable=self.training_rate, width=20)
        training_rate_panel.add(training_rate_entry)

        percent_label = Label(training_rate_panel, text="%", width=20, anchor="w", justify=LEFT)
        ...
        self.train_model_result_label = Label(
            training_rate_panel, textvariable=self.status
        )
        ...
        loadmodel_button.grid(row=0, column=2, padx=10, pady=5)

        # Gọi hàm cập nhật danh sách model (sẽ tạo bên dưới)
        ...
        designedby_label = Label(designedby_panel, text="Designed by: Nguyen Hoang Sang")
    ...
    def do_save_model(self):
        confirm = messagebox.askyesno("Confirm", "Do you really want to save the model?")
        if not confirm:
            return
        ...
        # Cập nhật dropdown model list sau khi lưu
        self.update_model_list()
        pass
    ...
    def do_load_model(self):
        ...
        if not os.path.exists(model_file):
            messagebox.showwarning("Warning", "Selected model file not found!")
            return

        self.lm = FileUtil.loadmodel(model_file)
        messagebox.showinfo("info", f"Loaded model: {model_file}")
